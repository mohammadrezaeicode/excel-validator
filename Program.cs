...
app.MapPost("/validate2", async (IFormFile file) =>
{
...
app.MapPost("/convert", async (IFormFile file, HttpContext ctx) =>
{
    ...
    if (file.Length < 100)
        return Results.BadRequest(new { error = "File is too small to be a valid Excel file." });
    ...
    try
    {
        ...
        var psi = new ProcessStartInfo
        {
            ...
            Arguments = $"--headless -env:UserInstallation=file://{userProfile} " +
        ...
        try { Directory.Delete(userProfile, true); } catch { }

        if (proc.ExitCode != 0)
            return Results.Problem($"LibreOffice conversion failed (exit {proc.ExitCode}).\n{stderr}");
        ...
        if (!File.Exists(outputPath))
        {
            ...
            if (found is null)
                return Results.Problem("Conversion produced no image. The file may be too corrupted to render.");
            outputPath = found;
        }
        ...
        return Results.File(imageBytes, "image/png", $"{Path.GetFileNameWithoutExtension(file.FileName)}.png");
    }
    finally
    {
